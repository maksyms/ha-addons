# Justfile Recipes Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a justfile with three recipes (`push`, `wait-and-update`, `pushdeploy`) for committing/pushing code and deploying HA add-ons.

**Architecture:** Single `justfile` at repo root. Shell recipes using `git`, `gh`, `jq`, and `ssh`. Configurable variables for HA host, poll interval, and timeout.

**Tech Stack:** just, bash (recipe shebang), git, gh CLI, jq, ssh
...
- [ ] **Step 1: Create the justfile with variables and all three recipes**
...
Run: `just --list`
Expected output showing all three recipes with their parameters.

- [ ] **Step 3: Dry-run the push recipe (check syntax only)**

Run: `just --dry-run push "test message"`
Expected: prints the shell commands without executing.

- [ ] **Step 4: Commit**

```bash
git add justfile
git commit -m "add justfile with push, wait-and-update, and pushdeploy recipes"
```
